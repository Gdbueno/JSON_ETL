...
if __name__ == '__main__':
    ...
    file= requests.get("https://jsonplaceholder.typicode.com/todos")
    
    data = file.json()
    userId = [x['userId'] for x in data if x.get('completed') is True]
    axis_x = list(set(userId))
    suma = [userId.count(x) for x in userId]
    axis_y =[suma[i] for i in range(len(suma)) if suma[i] != suma[i-1]]
    fig = plt.figure() 
    fig.suptitle('Titulos Completados', fontsize=20)
    width = 0.5
    ax = fig.add_subplot()
    ax.bar(axis_x, axis_y, width,color ='yellow', label='Usuario')
